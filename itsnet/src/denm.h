...
//typedef char itsnet_node_id[8] ; 

/*** update ***/

typedef enum hard_shoulder_status { /*bande d'arret */
...
/*typedef struct action_id {
	station_id_t originating_station_id; //header
...
typedef struct management_container {
	...
	itsnet_time_stamp detection_time; //header
	itsnet_time_stamp reference_time;
	e_termination termination /* OPTIONAL */;
	itsnet_position_vector event_position; //header
