...
int itsnet_packet_send(struct itsnet_packet* packet, char* dest_mac)
{
    ...
    /*btp_b_hdr* btp_b = (btp_b_hdr*)malloc(sizeof(btp_b_hdr)) ; 
    memset(btp_b, 0, sizeof(btp_b_hdr)) ; 
    btp_b-> destination_port = 2002 ; 
    btp_b-> destination_port_info = 0 ;*/
    ...
    if(OCB) {
        ...
        /* afficher les champs du packet reçu */ 
        
        
        
        
        
        /*if(packet->common_header.itsnet_header_type_subtype == HI_NIBBLE(itsnet_geobroadcast_id) {
        ...
        printf("taille prévu du packet itsnet : %d \n ", sizeof(struct itsnet_packet)) ; 
        
        printf("taille réel du packet : %d \n ", sizeof(*packet)) ; 
        
        
        memcpy((void*)(buffer + size_header), packet, sizeof(struct itsnet_packet));
        //memcpy((void*)(buffer + size_header + ITSNET_DATA_SIZE), btp_b, sizeof(btp_b_hdr)) ;  
...
int itsnet_btp_packet_send(struct itsnet_btp_packet* packet, char* dest_mac)
{
    ...
    if(OCB) {
        ...
        printf("taille prévu du packet itsnet : %d \n ", sizeof(struct itsnet_basic_header)+sizeof(struct itsnet_common_header) + sizeof(struct itsnet_geobroadcast_btp_t) +45) ; 
        
        printf("taille réel du packet : %d \n ", sizeof(*packet)) ; 
        
        
        memcpy((void*)(buffer + size_header), packet, sizeof(struct itsnet_btp_packet));
